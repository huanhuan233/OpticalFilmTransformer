...
if __name__ == '__main__':
    ...
    parser.add_argument('--layers', default=12, type=int, help='Encoder layers')
    ...
    parser.add_argument('--save_name', default='model_forward', type=str, help='First order folder')
    ...
    temp = [args.ratios, args.batch_size, args.max_lr, args.warm_steps, args.layers, args.head_num, args.d_model, args.d_ff]
    ...
    data = PrepareData(TRAIN_FILE, TRAIN_SPEC_FILE, args.ratios, DEV_FILE, DEV_SPEC_FILE, args.batch_size, args.spec_type)

    src_vocab = len(data.struc_word_dict)
    tgt_vocab = len(data.dev_spec[0])
